...
export class ApiCommand {
  ...
  private async call(client: IHttpClient) {
    switch (this.method) {
      case 'GET':
        return client.get(this.path, this.payload);
      case 'POST':
        return client.post(this.path, this.payload);
      case 'PUT':
        return client.put(this.path, this.payload);
      case 'DELETE':
        return client.delete(this.path);
      default:
