...
const AddEmployeeIntoDepartmentFilter: React.FC<Props> = ({ visible, onClose, onApply, current }) => {
    ...
    }, [visible, current?.employeeName, current?.position, current?.sortBy]);

    if (loading || !theme || !lang) return null;
    const S = makeStyles(theme);

    const applyAndClose = () => {
        onApply({ employeeName, position, sortBy}); // ⬅️ truyền thêm
        onClose();
    };
...
export default AddEmployeeIntoDepartmentFilter;
